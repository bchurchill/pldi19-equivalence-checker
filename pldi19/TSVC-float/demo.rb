...
@default_def_ins = "\"{ %rdi %rbp %rsp %rbx %r12 %r13 %r14 %r15 %xmm0 %xmm1 %xmm2 %xmm3 %xmm4 }\""
...
def validate(compiler1, compiler2, benchmark, dofork=false) 
  ...
  if benchmark == "sum1d" then
    live_outs = "\"{ %xmm0 %rbx %rsp %rbp %r12 %r13 %r14 %r15 }\""
  end
  ...
  if not @options[:novectorinv] then
    stoke_args.push_back("--vector_invariants")
  end
...
def update_options
  ...
  while ARGV[n].start_with?("-") do
    ...
    if ARGV[n] == "--no-vector-invariants" then
      @options[:novectorinv] = true
    end
    n = n+1
  end
  n
